...
public class Mushroom extends Entity {
    ...
    public void init() {
        ...
        for(int i = 0; i < LoadingScreen.TEXTURE_NUMBER_MUSHROOM ; i++){
            switch (i){
                ...
                case HIT: animations.add(new Animation(new TextureRegion(game.getAssetManager().get("ennemies/mushroom"+i+".png", Texture.class)),2 , 0.1F));break;
            }
        }
    }
    ...
    public void update(float deltaTime, float gravity, Camera camera, Stage stage) {
        ...
        if (target != null) {
            ...
            if (!invulnerable && xDistance < RANGE && xDistance > -RANGE) {
    ...
    public void takeAhit(double damage) {
        if (!invulnerable) {
            //System.out.println("aoutch, "+health+"hp left");
            health -= damage;
            flip = flipX;
